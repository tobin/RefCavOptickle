...
function param = paramTTFSS(ver, f)
  ...
  param.gainRF = 2; % should be 4?
  
  param.polesAB = [200e3 0; 20e3 0];
  param.zerosAB = [273 0; 30e3 0];
  
  param.coefPFD = 0.3;  % phase-frequency descriminate V/rad
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fast Path
  param.fastGain = 22; % in dB (800 => 22db)
  ...
  switch ver
    ...
    case 1
      ...
      % remove some anti-boost (so PZT path works better with AB on)
      param.zerosAB(1, :) = [4 * 273 0]; % R22, R24 = 124k -> 31k
      ...
      param.R30 = 2e3 + parRC(20e3, 33e-9, s); % FB, U8
      %param.R30 = 2e3; % FB, U8
      ...
      param.C23 = 100e-9;  % higher AC couple
    ...
    case 3
      ...
      % remove some anti-boost (so PZT path works better with AB on)
      param.zerosAB(1, :) = [40 * 273 0]; % R22, R24 = 124k -> 31k
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Fast Path
      % move PZT poles up, allowing wider band PZT
      param.R30 = 1e3 + parRC(20e3, 33e-9, s); % FB, U8
      %param.R30 = 2e3; % FB, U8
      param.C36 = 560e-12; % FB, U8
