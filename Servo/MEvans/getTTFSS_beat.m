...
%
% Example (parameter version 3 with the anti-boost enabled):
% [hOL, hCross, hEOM, hPZT, info] = getTTFSS(paramTTFSS(3, f), true, f);

function [hOL, hCross, hEOM, hPZT, info] = getTTFSS(param, enableAB, f)
  ...
  if enableAB
    filtRF = filtProd(filtRF, filtZPGQ(zerosAB, polesAB, 1, Inf));
  end
  hRF = sresp(filtRF, f);
  ...
  hOL(:, 1) = hDelayPZT .* coefPFD .* coefPZT .* hOut .* hPZT ./ s;
  hOL(:, 2) = hDelayEOM .* coefPFD .* coefEOM .* hHV .* hEOM;
  hOL(:, 3) = hOL(:, 1) + hOL(:, 2);
